...
function handleLPClick(lpData) {
    ...
    // LP 커버 시작 위치 = 항상 originalPosition
    const selectedCoverStartPos = lpData.originalPosition.clone();
    const selectedCoverTargetPos = new THREE.Vector3(-0.4, 0, 0);

    // LP 회전
    const startRotationY = lpData.originalRotation.y;
    const targetRotationY = lpData.originalRotation.y - Math.PI / 2.5;

    // Vinyl 시작 위치 = 항상 original
    const selectedVinylStartPos = lpData.vinyl ? lpData.vinylOriginalPosition.clone() : null;
    const selectedVinylTargetPos = lpData.vinyl ? new THREE.Vector3(0.9, 0, 0.5) : null;

    // Vinyl 회전
    const vinylStartRotationY = lpData.vinylOriginalRotation ? lpData.vinylOriginalRotation.y : 0;
    const vinylTargetRotationY = vinylStartRotationY - Math.PI / 2;
